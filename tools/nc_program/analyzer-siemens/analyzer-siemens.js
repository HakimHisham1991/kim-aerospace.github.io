...
function processFileContent(content) {
  ...
  lines.forEach((line, index) => {
    // Check for operation start - NEW VERSION using "Start of Operation"
    const operationMatch = line.match(/;\s*Start of Operation\s*:\s*(.+)/);
    if (operationMatch) {
      const operationName = operationMatch[1].trim();
    ...
    // Check for tool change (M06 or M6) - updated to be more flexible
    if (currentOperation && (line.includes('M06') || line.includes('M6'))) {
      // Find the tool number in the format T## before M06/M6
      const toolMatch = line.match(/(?:^|\s)(T\d+)(?=.*M06|.*M6)/);
      ...
      if (!currentOperation.toolName) {
        // Try to get tool name from comment after M06/M6
    ...
    if (currentOperation) {
        // Split line at semicolon and only process the part before it
        const codePart = line.split(';')[0];
        
        // Standard feedrate detection (F commands)
        const feedMatch = codePart.match(/(?:^|\s)F(\d+(?:\.\d+)?)(?:\s|$)/);
        if (feedMatch) {
            currentOperation.feedrates.add(feedMatch[1]);
        }
        
        // Special handling for CYCLE85 - only capture 7th parameter (FFR)
        const cycle85Match = codePart.match(/CYCLE85\(([^)]*)\)/);
        if (cycle85Match) {
            const params = cycle85Match[1].split(',');
            // We need at least 7 parameters (though some might be empty)
            if (params.length >= 7) {
                const ffr = params[6].trim();
                // Only add if not empty and not just a comma
                if (ffr && ffr !== '') {
                    currentOperation.feedrates.add(ffr);
                }
            }
        }
    }
