...
export function renderToolpath(displayContent, paths, minX, maxX, minY, maxY, currentScale, showArrows, drawBound, drawRange, forceAnimation = false, onComplete = null) {
  ...
  if (paths.length === 0 && displayContent.trim()) {
    ...
    updateCodeTables([], -1, 'G94'); // Reset table for empty paths with default feed mode
    return;
  }
  ...
  if (forceAnimation) {
    ...
    for (let i = 0; i <= animationState.currentPathIndex && i < paths.length; i++) {
      if (!paths[i].isValid || paths[i].mCode || (paths[i].mode !== 'G00' && paths[i].mode !== 'G01' && paths[i].mode !== 'G02' && paths[i].mode !== 'G03')) {
        console.debug(`Skipping path in non-animation loop: line ${paths[i].lineNumber + 1}, mode=${paths[i].mode}, mCode=${paths[i].mCode || 'none'}, gcodeFeedMode=${paths[i].gCodes?.feedMode || 'none'}, isValid=${paths[i].isValid}`);
        continue;
      }
    ...
    if (animationState.currentPathIndex < paths.length) {
      ...
      console.debug(`Non-animation: highlighted line ${paths[animationState.currentPathIndex].lineNumber + 1}, hasInvalidPath=${animationState.hasInvalidPath}, gcodeFeedMode=${paths[animationState.currentPathIndex].gCodes?.feedMode || 'none'}`);
      updateCodeTables(paths, animationState.currentPathIndex, paths[animationState.currentPathIndex].gCodes?.feedMode || 'G94');
    } else {
      updateCodeTables(paths, paths.length - 1, paths.length > 0 ? paths[paths.length - 1].gCodes?.feedMode : 'G94');
    }
