...
function processFileContent(content) {
  ...
  lines.forEach(line => {
    ...
    // Check for tool change (M06) - this is where we should get the tool number
    if (currentOperation && line.includes('M06')) {
      // Remove comments to avoid false matches
      const codeWithoutComments = line.replace(/\(.*?\)/g, '');
      
      // Find the last T command before M06 (most recent tool selection)
      const toolMatch = codeWithoutComments.match(/T(\d+)(?=.*M06)/);
      if (toolMatch) {
        // Remove leading zeros and add 'T' prefix
        const toolNum = toolMatch[1].replace(/^0+/, '');
        currentOperation.toolNumber = 'T' + toolNum;
      }
